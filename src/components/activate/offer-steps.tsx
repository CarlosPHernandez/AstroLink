...
export function useMentorOfferLoad() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        if (response.ok && data != null && Array.isArray(data.services)) {
        ...
        if (!cancelled) setLoadError('Could not load offer.');
      }
    })();
    return () => {
      cancelled = true;
  ...
  return { fields, setFields, loaded, loadedRef, loadError };
}
...
export function MentorOfferEditor({ hourlyRateDollars }: { hourlyRateDollars: number }) {
  const { fields, setFields, loaded, loadedRef, loadError } = useMentorOfferLoad();
  const [error, setError] = useState<string | null>(null);
  const [saved, setSaved] = useState(false);
  const [pending, setPending] = useState(false);
  const shownError = loaded ? error : loadError;
  const locked = pending || !loaded;
  ...
  return (
    <div className="activate-flow" data-testid="mentor-offer-settings">
      ...
      <OfferServicesFields value={fields} onChange={setFields} disabled={locked} />

      <h2 className="activate-section-title" style={{ marginTop: '2.5rem' }}>
        Hours
      </h2>
      <p className="activate-section-copy">{HOURS_COPY}</p>
      <OfferHoursFields value={fields} onChange={setFields} disabled={locked} />

      {shownError ? (
        <div className="mt-6">
          <FormAlert message={shownError} />
        </div>
      ) : null}
      {saved ? (
