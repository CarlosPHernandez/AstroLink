...
2. Set **`ENCRYPTION_KEY`** (`openssl rand -hex 32`) for session cookies. The app reads `ENCRYPTION_KEY` in `src/lib/crypto.ts`; `.env.example` also lists `SESSION_SECRET` but that name is not used by current code.
3. For local booking without Stripe: keep `SKIP_STRIPE_PAYMENTS=true` (default in `.env.example`).
4. Optional: `E2E_STUB_LLM=true` for stubbed briefings; `OPENAI_API_KEY` / `GEMINI_API_KEY` for real APX-02.

Cloud Agent VMs often inject only `NEXT_PUBLIC_SUPABASE_URL`. **`NEXT_PUBLIC_SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY` are required** for the landing page, preset logins, booking APIs, and Playwright E2E.
...
- **E2E:** Playwright `webServer` runs `npm run dev -- --hostname 127.0.0.1` with `SKIP_STRIPE_PAYMENTS=true` and `E2E_STUB_LLM=true`.
...
### Preset logins (mock auth)

On `/auth`, use simulation presets (e.g. Carlos mentee). Seed UUIDs must exist in Supabase (`20260531140100_seed_d1_dev.sql` applied on the linked project).
