...
Bilingual expert sessions (bidirectional live captions, post-call transcript panel, localized recap) are documented in `docs/d3-transcript-translation-roadmap.md`. Before editing transcript/translation code, read `skills/transcript-translation/SKILL.md`. Translate the *other* person; a missing STT language tag is not English. Mentees confirm caption language at join (`CaptionLanguageGate` → `POST /api/me/preferred-locale`); `translate-segment` honors that requested locale. Use `src/lib/transcript-translation/token-budget.ts` before sending transcript text to LLMs. Live captions use a dedicated `caption` LLM rate-limit scope (`LLM_MAX_CAPTION_*` in `.env.example`). Extend `src/lib/llm.ts` for APX-06 — do not add Vercel AI SDK without updating the AI SDK review doc.
