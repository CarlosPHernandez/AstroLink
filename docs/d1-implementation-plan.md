...
| **T4** — Daily (room + session page + capture) | Done (D1) | Private rooms, on-join meeting tokens, session gates. Webhook HMAC + `meeting.ended` → `post-session.ts`. See [reference/video-session.md](./reference/video-session.md). |
