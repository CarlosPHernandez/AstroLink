...
**Last updated:** 2026-05-31 — implementation landed on `main`; E2E dogfood pending.
...
| **T4** — Daily (room + session page + capture) | Partial | Real Daily embed + room create on fulfill; **Daily webhook → capture/APX-03 still stub** |
| **T5** — Tests (Vitest + contract tests) | Not started | |
...
- [ ] **T4** — Daily — Real room + session page (**capture on meeting end deferred**)
- [ ] **T5** — Tests — Vitest + agent/route contract tests
...
| Daily | `src/lib/daily.ts`, `src/app/session/[bookingId]/` |
